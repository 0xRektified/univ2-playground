...
contract UniswapV2Arb is Test {
    ...
    function swap(SwapParams calldata params) external {
        ...
        uint256[] memory amounts = IUniswapV2Router02(params.router0).swapExactTokensForTokens(
            params.amountIn, amountOut, path, address(this), block.timestamp
        );
        ...
        uint256[] memory amounts2 = IUniswapV2Router02(params.router1).swapExactTokensForTokens(
            amounts[1], totalOut, path, msg.sender, block.timestamp
        );
    ...
    function uniswapV2Call(address sender, uint256 /* amount0 */, uint256 /* amount1 */, bytes calldata data) external {
        ...
        // Send Profit to user
        uint256[] memory amounts2 = IUniswapV2Router02(params.router1).swapExactTokensForTokens(
            amounts[1], params.minProfit, path2, address(this), block.timestamp
        );
